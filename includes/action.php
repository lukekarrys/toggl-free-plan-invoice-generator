...
foreach($requestClients as $client) {
  ...
  if ($clients[$client['id']]) {
    ...
    $clientOptions .= '<option '.$selected.' value="invoice?id='.$client['id'].'">'.$client['name'].'</option>';
...
if ($currentClient) {
  ...
  foreach($requestProjects as $project) {
  	...
  	if ($project['client']['id'] == $currentClient['id']) {
  		...
  		if ((isset($_POST['select_project']) && $_POST['select_project'] == $project['id']) || (!isset($_POST['select_project']) && $projectCount == 0)) {
  		  $selected = 'selected="selected"';
  		  $currentProject = $project;
  		}
  		
  		$projectOptions .= '<option '.$selected.' value="'.$project['id'].'">'.$project['name'].'</option>';		
  		$projectCount++;
